...
function Room:generateHeart(entity)
    ...
    for i = index, #self.objects do
        local heart = self.objects[i]

        -- define a function for the heart that when consumed raises health
...
function Room:update(dt)
    ...
    for i = #self.entities, 1, -1 do
        ...
        if entity.health <= 0 then
            ...
            if self.entitiesCheck[i] then
                if math.random(7) == 1 and self.player.health < 5 then
                    self:generateHeart(entity)
                end
            self.entitiesCheck[i] = false
            end
    ...
    for k, object in pairs(self.objects) do
        ...
        if self.player:collides(object) then
            ...
            elseif not object.consumable then
                object:onCollide()
            end

            if object.solid and object ~= self.pickedPot then
...
function Room:render()
    ...
    -- love.graphics.printf('Dead: ' .. tostring(dead), 0, 0, VIRTUAL_WIDTH, 'center')
end