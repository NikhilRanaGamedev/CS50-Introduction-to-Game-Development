...
function PlayState:enter(params)
    ...
    self.level = LevelMaker.generate(params.levelWidth, 10, params)
    ...
    for i = 1, 100 do
        if self.tileMap.tiles[7][i].id == TILE_ID_GROUND then
            playerX = i + (16 * (i - 1))
            break
        end
    end
...
function PlayState:render()
    ...
    --love.graphics.printf('Tile Value: ' .. tostring(self.tileMap.tiles[7][1].id), 0, 0, VIRTUAL_WIDTH, 'center')
end
