...
function AlienLaunchMarker:update(dt)
    ...
    if not self.launched then
        ...
        if love.keyboard.wasPressed('space') and not self.alien.collided then
            self.alien1 = Alien(self.world, 'round', xPos, yPos - 50, 'Player')
            self.alien1.body:setLinearVelocity(xVel, yVel)
            self.alien1.fixture:setRestitution(0.4)
            self.alien1.body:setAngularDamping(1)

            self.alien2 = Alien(self.world, 'round', xPos, yPos + 50, 'Player')
            self.alien2.body:setLinearVelocity(xVel, yVel)
...
function AlienLaunchMarker:render()
    if not self.launched then
        ...
        if self.alien1 ~= nil and self.alien2 ~= nil then
            self.alien1:render()
            self.alien2:render()
        end
    end
